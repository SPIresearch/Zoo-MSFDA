...
class OursEnsemble(nn.Module):
    def __init__(self, netF_list,netC_list,classes, source,optimized_models,tran_tran,aug_set,no_update_set,model_features,model_outs=[]):
        """
        ...
        self.loss=0
        self.tran_tran=torch.tensor(tran_tran, dtype=torch.float32) #nn.Parameter(3*torch.tensor(tran_tran, dtype=torch.float32))
    ...
    def forward_mix(self, x,idx,mix_idx,l):
        ...
        for i in range(self.optimized_models):
            ...
            if i not in self.no_update_set:
                ...
                self.features_all[i] =l * input_a + (1 - l) * input_b #self.model_features[i][idx].cuda()
