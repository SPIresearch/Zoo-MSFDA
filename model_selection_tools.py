...
def ce_nce_pse_principle(all_output, all_label, all_fea, struc_pse_label, args):
    ...
    # struc_pse_label=obtain_label(all_output, all_label, all_fea, args)
    # struc_pse_label=torch.from_numpy(struc_pse_label).long()
    struc_pse_label1 = get_one_hot(
        all_output, struc_pse_label
    )  # np.array(struc_pse_label)
    predict1 = get_one_hot(all_output, predict)  # np.array(predict)

    with torch.no_grad():
        loss1 = nn.CrossEntropyLoss(reduction="none")(all_output, struc_pse_label)
        loss2 = nn.CrossEntropyLoss(reduction="none")(
            predict1, struc_pse_label1
        )  # +0.001
...
def SUTE_principle(all_output, all_label, all_fea, struc_pse_label, args):
    ...
    SC = SC_cal(
        predict, struc_pse_label
    )  # +0.1*entropy_pse.item()#(torch.mean(Entropy(struc_pse_label1))).item()
    IC = -Entropy
    # loss=loss2-0.1*Entropy#loss1#loss2-0.2*loss1#;loss2#-loss1
    return SC, IC, GD, pred_acc
    ...
    if file_contents:
        ...
        with open(transferability_output_filename, "w") as output_file:
            for line in file_contents:
                ...
                )  # reduction='none'表示不进行汇总，而是保留每个样本的损失值
                plt.figure()
                # 画出分布图
                plt.hist(
                    losses.numpy(), bins=20
                )  # 将Tensor转换为NumPy数组，并画出直方图
                plt.xlabel("Cross-Entropy Loss")
                plt.ylabel("Frequency")
                # plt.title('Distribution of Cross-Entropy Loss')
                # 保存图片
