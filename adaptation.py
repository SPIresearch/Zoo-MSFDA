...
def calculate_loss(output,div_pse,div_mask):
    ...
    #entropy_loss =MI(output)
    #classifier_loss = nn.CrossEntropyLoss()(output, pse)#+ 0.3*F.kl_div(torch.log_softmax(output,1),torch.softmax(pse2,1))#nn.CrossEntropyLoss()(output, pse2)
...
def data_load(args): 
    ...
    dsets["target"] = ImageList_idx_fast(txt_tar, transform1=image_train(256,224),transform2=image_train(518,518))
    ...
    dsets["test"] = ImageList_idx_fast(txt_test, transform1=image_test(256,224),transform2=image_test(518,518))
    dset_loaders["test"] = DataLoader(dsets["test"], batch_size=3*train_bs, shuffle=False, num_workers=args.worker, drop_last=False,pin_memory=True)
...
def model_initial(net,update_F_list,update_C_list,source,optimized_models,path_list):
    param_group = []
...
def model_initial_tc(net,update_F_list,update_C_list,source,optimized_models,path_list):
    param_group = []
    exclude_names=["netF","netC"]
...
def train_target(args,model,optimizer,tran_tran,selected_pred_class,mask,pse_tran):
    ...
    while iter_num < max_iter:
        ...
        features_test,outputs_test =model.forward_mix(inputs_test,tar_idx,mix_idx,l)
...
def obtain_label(loader, model, args):
    ...
    with torch.no_grad():
        ...
        for _ in range(len(loader)):
            ...
            if start_test:
                ...
                #for i in range(model.source):
                #    all_fea_list[i] = fea_list[i].float().cpu()
                #    all_out_list[i] = out_list[i].float().cpu()
                all_output = outputs.float().cpu()
                all_label = labels.float()
                start_test = False
            else:
                ...
                all_label = torch.cat((all_label, labels.float()), 0)
                #for i in range(model.source):
                    #all_fea_list[i] = torch.cat((all_fea_list[i], fea_list[i].float().cpu()), 0)
                    #all_out_list[i] = torch.cat((all_out_list[i], out_list[i].float().cpu()), 0)
    ...
    # stacked_predictions = torch.stack([pred_list[model.source-i] for i in range(model.source) if i != 1], dim=0)
    # #pdb.set_trace()
    # # 对每个样本，选择在每个类别上出现次数最多的类别作为伪标签
    # pseudo_labels, _ = torch.mode(stacked_predictions, dim=0)
    # accuracy = torch.sum(torch.squeeze(pseudo_labels).float() == all_label).item() / float(all_label.size()[0])
    # print('Vote Accuracy = {:.2f}% '.format(accuracy * 100))
...
def aggregation_model(netF,netC,aug,optimized_models,model_path,tran_tran,model_features,model_outputs,args):
    update_F_list=[]
    update_C_list=[]
    ...
    model=OursEnsemble(netF,netC,args.class_num,len(netF),optimized_models,tran_tran,aug,feature_encoder_wo_update_set,model_features,model_outputs)
    #pdb.set_trace()
    for i in range(optimized_models):
        # if model_name_set[i]!='vit_h_14':
        #     update_F_list.append(i)
        #else:
        update_C_list.append(i)
    

    model,optimizer=model_initial_tc(model,update_F_list,update_C_list,len(netF),optimized_models,model_path)
...
def calculate_MI_cls_loss(output,pse):
    ...
    entropy_loss =ent(output)
    classifier_loss = nn.CrossEntropyLoss()(output, pse)
    loss=entropy_loss#+0.3*classifier_loss
    return loss
def pse_training(outputs,all_label,args):
    ...
    selected_pred_class = torch.gather(pred_class, 1, selected_model_indices.view(-1, 1))
    ...
    total_samples = mask.sum().item()  # Total number of samples where mask == 1

    accuracy = correct_predictions / total_samples

    print("Accuracy for samples where mask == 1:", accuracy)
    return selected_pred_class,mask#pred_label.astype('int')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--transferability_save_path', type=str, default="./model_sort/")
    ...
    for tt in range(len(names)):
        ...
        model_config_file = args.model_config_path+args.config_file+'/'+str(args.t)+'.txt'
        ...
        transferability_output_filename = args.transferability_save_path+args.config_file+'/'+str(args.t)+'.txt'

        # The Spearman correlation will be obtained in the following function
        all_output=transfer_calcualte_for_individual_models(model_config_file,feature_path,transferability_output_filename,method,args=args)
        transfer_set,div_set,tran_tran,div_tran,sorted_div_tran,tran_output,div_output,sorted_model_names,pse_tran,label=model_selection_function(feature_path,transferability_output_filename,all_output,method,args)

        

        if args.diversity:
            transfer_set.append(div_set[0])#.insert(1, div_set[0])
            tran_tran.append(sorted_div_tran[0])#.insert(1, sorted_div_tran[0])
         
        tran_tran=np.array(tran_tran)
        tran_tran=torch.from_numpy(tran_tran)
        other_set=[selected_model for selected_model in sorted_model_names if selected_model not in transfer_set]
        ...
        for i,selected_model in enumerate(transfer_set):
            ...
            dset,model_structure,model_name,source,target=split_dset_structure_model_source_target(selected_model)
            ...
            model_path.append(get_source_model_path(args.source_pth_save_dir,dset,model_name,source))
        ...
        model,optimizer=aggregation_model(netF,netC,aug_set,optimized_models,model_path,tran_tran,model_features,model_outputs,args)
        
     
        
        args.output_dir = osp.join(args.output_dir, args.dset,args.config_file, names[args.t][0].upper())
        ...
        #pse_others=torch.from_numpy(pse_others)
        model=train_target(args,model,optimizer,tran_tran,selected_pred_class,mask,pse_tran)
...
def calculate_MI_cls_loss(output, pse):
    ...
    Entropy = ent(output)
    classifier_loss = nn.CrossEntropyLoss()(output, pse)
    loss = Entropy  # +0.3*classifier_loss
    return loss
